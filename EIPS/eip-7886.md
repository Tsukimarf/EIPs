...
6. `gas_used`: Total gas used by transactions in the block (validated during block validation)
7. `blob_gas_used`: Total blob gas used by transactions in the block (validated during block validation)
...
7. Total inclusion gas (using the maximum of intrinsic gas and EIP-7623 floor cost plus calldata- and blob-gas) MUST not exceed the block's declared gas_used.
...
1. Clients MUST create a block-level snapshot via `begin_transaction` before any transaction execution occurs.
...
3. Transactions are executed sequentially until an error occurs or all complete successfully.
...
7. Withdrawal processing and general purpose requests are skipped when execution is reverted.

8. The execution outputs (`last_receipt_root`, `last_block_logs_bloom`, `last_requests_hash`, `last_execution_reverted`) are updated in the chain state based on the execution results, and will be verified in subsequent blocks.
