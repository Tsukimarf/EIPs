...
This validation step ensures the block can be attested to without executing its transactions, as all structural and balance constraints are checked.
...
5. If gas mismatch is detected, clients MUST roll back to the snapshot taken before the block execution and produce empty execution outputs.
